...
The plot shows that a possibly very weak relationship between max confinement days and age.
...
```
Some defendants have more than 50 dockets associated with them. A few even have more than 150 dockets, yet the total days of confinement of these extreme docket holders are zeros. Not sure whether these are plausible numbers.
...
dockets_data %>% 
  ...
  geom_smooth(method = "lm", se = FALSE)
```
For defendants whose number of prior dockets are less than 50, there appear a weak relationship between number of prior dockets and total confinement days.
...
dockets_data %>% 
  ...
  geom_smooth(method = "lm", se = FALSE)
```
For defendants whose number of prior dockets are less than 50, there appear a weak relationship between number of prior dockets and max confinement days.
...
dockets_data %>% 
  ...
  geom_smooth(method = "lm", se = FALSE)
```
The plot shows a moderate relationship between number of grade F1 offenses and total confinement days.
