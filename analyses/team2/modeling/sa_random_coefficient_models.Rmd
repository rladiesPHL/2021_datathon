...
```{r}
ranef(judge_log_total_m2)$judge_id
```

```{r}
judge_log_total_ranef_m2 <- tibble(ranef(judge_log_total_m2)$judge_id) %>%
  rename(ranef_intercept = `(Intercept)`) %>% 
  mutate(judge_id = row_number())
```
...
```
```{r}
ranef(judge_log_max_m2)$judge_id
```

```{r}
judge_log_max_ranef_m2 <- tibble(ranef(judge_log_max_m2)$judge_id) %>%
  rename(ranef_intercept = `(Intercept)`) %>% 
  mutate(judge_id = row_number())
```
