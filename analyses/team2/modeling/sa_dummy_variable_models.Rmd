...
```
### on full dockets: model that includes grade severity
...
Diagnostics
```{r}
qqnorm(qresid(judge_total_m3), main = "Quantile residuals (set 1)", las=1); qqline(qresid(judge_total_m3))
```
```{r}
qqnorm(qresid(judge_total_m3), main = "Quantile residuals (set 2)", las=1); qqline(qresid(judge_total_m3))
```
