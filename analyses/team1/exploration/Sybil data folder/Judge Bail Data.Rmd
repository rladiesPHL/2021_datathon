...
# Create new dataset by merging both datasets
 jatdata1 <- merge(newdefdocid3, defdoc3, by="docket_id")
```
...
```{r}
df5 <- completerecords %>% 
  group_by(defendant_id, race, action_type_name) %>%
  filter(factor(Judge) ==  "Rayford Means") %>% 
  summarise(counts = n()) %>%
  mutate(race=fct_reorder(race, counts))
df5
...
```{r}
orca(b, file = "bail.png")
```


```{r}
b <- plotly::ggplotly(RM12 + scale_y_continuous(limits = c(0, 250)))

htmlwidgets::saveWidget(b, "bail.html")
```
